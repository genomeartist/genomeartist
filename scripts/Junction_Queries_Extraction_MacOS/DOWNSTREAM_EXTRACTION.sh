...
do
 ...
 then
  ...
  START=$coordinate1
  STOP=$( expr $coordinate2 + $LengthFL )
...
gsed -i -e "s/-ins/\-ins Contig length: $Contig_Length. Coordinates: $coordinate1-$coordinate2 (TE length: $TE_Length). Strand: $strand. Range: $START:$STOPh. Total length: $Length1. /g" $i
...
if [ $strand = minus ]
 then
  ...
  then
  START=$( expr $coordinate2 - $LengthFL )
  fi
  STOP=$coordinate1
TE_Length=$( expr $qend - $qstart + 1 )  
