...
gsed -i -e "s/-ins/\-ins_only_IR5_TG. Contig length: $Contig_Length. Coordinates: $coordinate1-$coordinate2 (TE length: $TE_Length). Strand: $strand. Range: $START:$STOPh. Total length: $Length1. /g" $i
...
if [ $STOP -gt $STOPh ]
then
gsed -i -e "s/-ins_only_IR5_TG./\-ins_only_IR5_TG. ALERT! /g" $i
fi
